/**
 * Import Re:Zero S02E26-E50 learning CSV packages into Supabase (jps).
 * Usage:
 *   set SUPABASE_ACCESS_TOKEN=sbp_...
 *   set DRY_RUN=1&& node scripts/import-rezero-s02e26-e50.mjs
 *   set DRY_RUN=0&& node scripts/import-rezero-s02e26-e50.mjs
 */
...
const PACKAGES = [
  join(ROOT, 'rezero_s02_ep26_ep50_learning_csv_import_final_best_available', 'rezero_s02_ep26_ep50_learning_csv_import_final_best_available'),
]
const TABLE_ORDER = [
...
const INT_COLS = new Set([
  ...
  'start_line', 'end_line', 'line_count',   'occurrence_count', 'source_line_no', 'sort_order', 'plan_slot',
  'total_occurrences', 'episode_count',
])
const DELETE_ORDER = [
...
function buildUniqueVocabInsertRows(bundles, idRemap) {
  const byId = new Map()
  for (const bundle of bundles) {
    for (const row of bundle.rows.learning_vocab_items) {
      const cid = remapId(row.id, idRemap)
      if (!byId.has(cid)) {
        const parsed = parseRow('learning_vocab_items', row, idRemap)
        parsed.id = cid
        byId.set(cid, parsed)
      }
    }
...
function parseJsonbValue(key, val) {
  ...
  if (key.endsWith('_ids') && s.includes('|')) {
    return s.split('|').map((x) => x.trim()).filter(Boolean)
  }
  if (key === 'example_line_nos') {
    if (s.includes('|')) return s.split('|').map((x) => Number(x.trim())).filter((n) => !Number.isNaN(n))
    const n = Number(s)
    return Number.isNaN(n) ? [s] : [n]
  }
  throw new Error(`Invalid JSON in ${key}: ${s.slice(0, 80)}`)
}
...
function parseRow(table, row, idRemap) {
  ...
  for (const [key, val] of Object.entries(row)) {
    ...
    if (JSONB_COLS.has(key)) {
      let parsed = parseJsonbValue(key, val)
    ...
    } else if (key === 'plan_slot') {
      const slot = String(val).toLowerCase()
      out[key] = ({ core: 1, shadowing: 2, review: 3 })[slot] ?? Number(val)
...
function dedupeGrammarPoints(rows) {
  const idRemap = {}
  const byKey = new Map()
  for (const row of rows) {
    const key = `${row.work_slug}\0${row.episode}\0${row.pattern}\0${row.ja_example}`
    if (!byKey.has(key)) {
      byKey.set(key, row)
    } else {
      idRemap[row.id] = byKey.get(key).id
    }
  }
  return { rows: [...byKey.values()], idRemap }
}

function remapGrammarInPlans(plans, grammarIdRemap) {
  return plans.map((row) => {
    const out = { ...row }
    if (Array.isArray(out.grammar_point_ids)) {
      out.grammar_point_ids = [...new Set(out.grammar_point_ids.map((id) => grammarIdRemap[id] ?? id))]
    }
    return out
  })
}
...
function dedupeSentences(rows) {
  ...
  const byKey = new Map()
  for (const row of rows) {
    const key = `${row.work_slug}\0${row.episode}\0${row.source_line_no}`
    if (!byKey.has(key)) {
      byKey.set(key, row)
      continue
    }
    const kept = byKey.get(key)
    const preferNew = (row.recommended_shadowing && !kept.recommended_shadowing)
      || (row.recommended_shadowing === kept.recommended_shadowing && (row.sort_order ?? 999) < (kept.sort_order ?? 999))
    if (preferNew) {
      idRemap[kept.id] = row.id
      byKey.set(key, row)
    } else {
      idRemap[row.id] = kept.id
    }
  }
  return { rows: [...byKey.values()], idRemap }
}

function remapSentenceIdsInPlans(plans, sentenceIdRemap) {
  return plans.map((row) => {
    const out = { ...row }
    if (Array.isArray(out.shadowing_sentence_ids)) {
      out.shadowing_sentence_ids = [...new Set(out.shadowing_sentence_ids.map((id) => sentenceIdRemap[id] ?? id))]
    }
    return out
  })
}

function dedupeExercises(rows) {
  const idRemap = {}
  const byKey = new Map()
  for (const row of rows) {
    const key = `${row.work_slug}\0${row.episode}\0${row.exercise_type}\0${row.prompt}`
    if (!byKey.has(key)) {
      byKey.set(key, row)
    } else {
      idRemap[row.id] = byKey.get(key).id
...
function remapExerciseIdsInPlans(plans, exerciseIdRemap) {
  return plans.map((row) => {
    const out = { ...row }
    if (Array.isArray(out.exercise_ids)) {
      out.exercise_ids = [...new Set(out.exercise_ids.map((id) => exerciseIdRemap[id] ?? id))]
    }
    return out
  })
}
...
function validateReferences(bundles, idRemap) {
  const errors = []
  const episodeIds = new Set(bundles.map((b) => b.rows.episodes[0].id))
  ...
  for (const bundle of bundles) {
    const epId = bundle.rows.episodes[0].id
    ...
    for (const row of bundle.rows.episode_learning_plans) {
      for (const col of Object.keys(refSets)) {
        if (!row[col]) continue
        const ids = parseJsonbValue(col, row[col])
...
function printReport(ctx) {
  ...
  console.log('DRY_RUN REPORT - Re:Zero S02E26-E50 learning CSV import')
  ...
  console.log(`--- existing rows to delete (EP${EP_MIN}-${EP_MAX}) ---`)
...
async function generateImportReport(token, backupDir, expected) {
  ...
  w('# Re:Zero S02E26-E50 Import Report')
  w('')
  ...
  w(`## Table row counts (EP${EP_MIN}-${EP_MAX}, work_slug=re-zero)`)
  w('')
  ...
  const ep125 = await runSql(token, `select count(*)::int as cnt from public.episodes where work_slug='${WORK_SLUG}' and episode between 1 and 25`)
  w(`## EP01-25 episodes preserved: ${ep125[0].cnt} rows (unchanged baseline)`)
  w('')

  const reportPath = join(ROOT, 'output', 'learning', 'import_report_s02e26_e50.md')
...
async function runLiveImport(token, parsed, vocabRows, meta) {
  ...
  const backupDir = join(ROOT, 'backups', `rezero_s02e26_e50_before_import_${ts}`)
  ...
  const incomingVocabIds = new Set(parsed.learning_vocab_occurrences.map((r) => r.vocab_item_id))

  for (const table of DELETE_ORDER) await runSql(token, `delete from public.${table} where ${SCOPE}`)

  const deletableVocabIds = oldVocabIds.filter((id) => !incomingVocabIds.has(id))
  if (deletableVocabIds.length) {
    const idsSql = deletableVocabIds.map((id) => `'${id.replace(/'/g, "''")}'`).join(', ')
...
async function main() {
  ...
  const { idRemap, vocabRows: _rawVocabRows, surfaceCsvIds } = buildVocabMaps(bundles, existingBySurface)
  const vocabRows = buildUniqueVocabInsertRows(bundles, idRemap)
  const parsed = collectParsedRows(bundles, idRemap)
  const { rows: grammarRows, idRemap: grammarIdRemap } = dedupeGrammarPoints(parsed.learning_grammar_points)
  parsed.learning_grammar_points = grammarRows
  parsed.episode_learning_plans = remapGrammarInPlans(parsed.episode_learning_plans, grammarIdRemap)
  const { rows: sentenceRows, idRemap: sentenceIdRemap } = dedupeSentences(parsed.learning_sentences)
  parsed.learning_sentences = sentenceRows
  parsed.episode_learning_plans = remapSentenceIdsInPlans(parsed.episode_learning_plans, sentenceIdRemap)
  const { rows: exerciseRows, idRemap: exerciseIdRemap } = dedupeExercises(parsed.learning_exercises)
  parsed.learning_exercises = exerciseRows
  parsed.episode_learning_plans = remapExerciseIdsInPlans(parsed.episode_learning_plans, exerciseIdRemap)
  if (Object.keys(grammarIdRemap).length) {
    console.log(`grammar_dedup: dropped ${Object.keys(grammarIdRemap).length} duplicate rows by (work_slug, episode, pattern, ja_example)`)
  }
  if (Object.keys(sentenceIdRemap).length) {
    console.log(`sentence_dedup: dropped ${Object.keys(sentenceIdRemap).length} duplicate rows by (work_slug, episode, source_line_no)`)
  }
  if (Object.keys(exerciseIdRemap).length) {
    console.log(`exercise_dedup: dropped ${Object.keys(exerciseIdRemap).length} duplicate rows by (work_slug, episode, exercise_type, prompt)`)
  }
