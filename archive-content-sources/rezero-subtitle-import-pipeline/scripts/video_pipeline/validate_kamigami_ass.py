...
from ass_parser import parse_ass_to_index, read_text_auto
from clipper import ms_to_seconds, probe_video, require_ffmpeg
from matcher import LearningSentence, match_sentence, normalize_for_match
...
def ass_clip_range(ass_rows, match, probe_ms):
    if match.subtitle_start_ms is None:
        return None, None
    clip_start = max(0, match.subtitle_start_ms - PRE_ROLL_MS)
    clip_end = min(probe_ms or match.subtitle_end_ms + POST_ROLL_MS, match.subtitle_end_ms + POST_ROLL_MS)
    if match.subtitle_index is None:
        return clip_start, clip_end
    idx = match.subtitle_index
    for i in range(idx - 1, 0, -1):
        ...
        if gap > MAX_BACK_GAP_MS:
            break
        raw = prev.ja_text.strip()
        norm = normalize_for_match(raw)
        if not norm or len(norm) > 12:
            break
        if raw.endswith("\u2026") or raw.endswith("...") or len(norm) <= 5:
            clip_start = max(0, prev.start_ms - PRE_ROLL_MS)
            idx = i
        else:
            break
    return clip_start, clip_end
...
def run(args: argparse.Namespace) -> int:
    ...
    ass_rows = parse_ass_to_index("re-zero-s01e01", ass_path)
    ...
    report = []
    test_ids = {f"re-zero-s01e01-sentence-{i:03d}" for i in (1, 2, 3, 25)}

    print("=== Kamigami ASS vs Netflix SRT (episode 1) ===")
    print(f"{'#':>2}  {'dStart':>7}  {'ASS':>23}  {'SRT':>23}  match")
    print("-" * 95)
    ...
    for item in sorted(sentences, key=lambda x: x["sort_order"]):
        ...
        ass_match = match_sentence(LearningSentence(sid, item["sort_order"], ja, "", src), ass_rows)
        s_line, e_line, _ = expand_range(srt_cues, src, ja)
        srt_start = srt_cues[s_line]["start_ms"]
        srt_end = srt_cues[e_line]["end_ms"]
        row = {
            "sentence_id": sid,
            "sort_order": item["sort_order"],
            "ja_text": ja,
            "ass_status": ass_match.match_status,
            "ass_reason": ass_match.match_reason,
            "ass_start_ms": ass_match.subtitle_start_ms,
            "ass_end_ms": ass_match.subtitle_end_ms,
            "srt_start_ms": srt_start,
            "srt_end_ms": srt_end,
        }
        if ass_match.subtitle_start_ms is not None:
            ds = ass_match.subtitle_start_ms - srt_start
            de = ass_match.subtitle_end_ms - srt_end
            row["delta_start_ms"] = ds
            row["delta_end_ms"] = de
            print(
                f"{item['sort_order']:2d}  {ds:+6d}ms  "
                f"{ms_to_ts(ass_match.subtitle_start_ms)}-{ms_to_ts(ass_match.subtitle_end_ms)}  "
                f"{ms_to_ts(srt_start)}-{ms_to_ts(srt_end)}  "
                f"{ass_match.match_reason}"
            )
        else:
            print(f"{item['sort_order']:2d}     N/A  UNMATCHED  {ja[:40]}")

        if sid in test_ids and ass_match.clip_eligible:
            clip_start, clip_end = ass_clip_range(ass_rows, ass_match, probe.duration_ms)
            out_file = out_dir / f"{sid}_kamigami.mp3"
            ok, err = extract_audio(ffmpeg, video, out_file, clip_start, clip_end)
            row["validate_clip"] = out_file.name
            row["clip_start_ms"] = clip_start
            row["clip_end_ms"] = clip_end
            row["clip_ok"] = ok
            row["clip_error"] = err
            if ok:
                print(f"    -> clip OK {clip_end - clip_start}ms  {out_file.name}")

        report.append(row)

    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "alignment_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    matched = sum(1 for r in report if r.get("ass_status") == "matched")
    print(f"\nASS matched {matched}/{len(report)} sentences")
    print(f"Report: {out_dir / 'alignment_report.json'}")
    return 0
