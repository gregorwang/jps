/**
 * Re:Zero EP07 CSV -> Supabase SQL generator + optional executor
 * Usage:
 *   node scripts/import-rezero-ep07.mjs --write-batches
 *   SUPABASE_ACCESS_TOKEN=... node scripts/import-rezero-ep07.mjs --run
 */
...
const CSV_DIR = join(__dirname, '..', 'rezero_ep07_learning_csv_import')
...
function generateImportSql() {
  const statements = []
  statements.push(`-- DELETE EP${String(EPISODE).padStart(2, '0')} scoped rows`)
  for (const table of [
    'learning_exercises',
    'learning_sentences',
    'learning_grammar_points',
  ...
  return statements.filter((s) => s && !s.startsWith('--') || s.startsWith('DELETE') || s.startsWith('INSERT'))
}
...
const statements = generateImportSql().filter((s) => s.startsWith('DELETE') || s.startsWith('INSERT'))

if (writeBatches) {
  mkdirSync(BATCH_DIR, { recursive: true })
  statements.forEach((sql, i) => {
    writeFileSync(join(BATCH_DIR, `${String(i + 1).padStart(3, '0')}.sql`), sql + '\n', 'utf8')
  })
